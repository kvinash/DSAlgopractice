dlet minmax = (A) => {
        //create four corresponding array to each array to store 
        // nearest left greater index, (jaha tak wo greater hai) 
        // nearest right greater index 
        // nearest right smaller index (jaha tak wo smaller hai)
  ...
  //to finnd the LG
        for(let i=0;i<A.length;i++){
            while(solStack.length>0 && solStack[solStack.length-1].value<A[i]){
                solStack.pop();
            }
            if(solStack.length>0 &&  solStack[solStack.length-1].value > A[i]){
             
                    LG[i]= solStack[solStack.length-1].index;
               solStack.push({value : A[i], index : i});
            } 
        if(solStack.length<1){
          LG[i] = -1;
          solStack.push({value : A[i], index : i});
        } 
 ...
 //to find the RG
        for(let i=A.length-1;i>-1;i--){
            ...
            if(solStack.length>0 &&  solStack[solStack.length-1].value > A[i]){
                    ...
                    RG[i]= solStack[solStack.length-1].index;
               solStack.push({value : A[i], index : i});
            } 
        if(solStack.length<1){
          RG[i] = A.length;
  ...
  //to finnd the RS
        for(let i=0;i<A.length;i++){
            ...
            if(solStack.length>0 &&  solStack[solStack.length-1].value < A[i]){
                    ...
                    RS[i]= solStack[solStack.length-1].index;
               solStack.push({value : A[i], index : i});
            } 
        if(solStack.length<1){
          RS[i] = -1;
  ...
  //to finnd the LS
        for(let i=A.length-1;i>-1;i--){
            ...
            if(solStack.length>0 &&  solStack[solStack.length-1].value < A[i]){
                    ...
                    LS[i]= solStack[solStack.length-1].index;
               solStack.push({value : A[i], index : i});
            } 
        if(solStack.length<1){
          LS[i] = A.length;
...
let totalSum = 0;
for(let i=0;i<A.length;i++){
  console.log(A[i], i, LG[i], RG[i], RS[i], LS[i]);
  totalSum = eval(totalSum + A[i] * eval((i-LG[i])*(RG[i]-i)) - A[i]*eval((i-LS[i])*(RS[i]-i)))
}
 console.log(totalSum%1000000009)
  return totalSum%1000000009;
